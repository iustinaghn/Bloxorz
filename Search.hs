...
data Node s a = N {stare :: s, actiune :: Maybe a, parinte :: Node s a, adancime :: Int, copii :: [Node s a]} | NodeNil
    deriving (Eq, Show)
{-
    *** TODO ***
...
nodeState (N stare actiune parinte adancime copii) = stare
...
limitedDfs (N stare actiune parinte adancime copii) a = undefined
...
path :: Node s a -> [(a, s)]
path (N stare Nothing parinte adancime copii) = []
path (N stare actiune parinte adancime copii) = (extractPath parinte) ++ [(M.fromJust actiune, stare)]

extractPath :: Node s a -> [(a, s)]
extractPath (N stare actiune parinte adancime copii) = path (N stare actiune parinte adancime copii)
